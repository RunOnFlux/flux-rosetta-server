...
concurrently --restart-tries 1000000 \
        "DATA_PATH=\"$DATA_DIR\" CONNECTION=\"$CONNECTION\" PORT=\"$ONLINE_PORT\" FLUX_NETWORK=\"$NETWORK\" FLUX_VERSION=\"$VERSION\" npm run start > online.log" \
        "DATA_PATH=\"$DATA_DIR\" CONNECTION=\"$CONNECTION\" PORT=\"$OFFLINE_PORT\" OFFLINE_MODE=1 FLUX_NETWORK=\"$NETWORK\" FLUX_VERSION=\"$VERSION\" npm run start > offline.log"
